...
class Zipformer2(EncoderInterface):
    ...
    def forward(
        ...
        x_full = self._combine_encoder_output(outputs)
        x = self.downsample_output(x)
        seq_len, n_blocks, b, d = x_full.shape
        x_full = self.downsample_output(
            x_full.reshape(seq_len, n_blocks * b, d)
        ).reshape(-1, n_blocks, b, d)
        x_full = x_full.permute(1, 0, 2, 3)  # (n_blocks, seq_len, bz, dim)
    ...
    def _combine_encoder_output(self, outputs: List[Tensor]):
        num_encoders = len(self.encoder_dim)
        assert len(outputs) == num_encoders
        output_dim = max(self.encoder_dim)
        output_pieces = []
        for i in range(num_encoders):
            seq_len, batch_size, embedding_dim = outputs[i].shape
            this_output = torch.zeros(
                [seq_len, batch_size, output_dim],
                dtype=outputs[i].dtype,
                device=outputs[i].device,
            )
            this_output[:, :, :embedding_dim] = outputs[i]
            output_pieces.append(this_output)
        output_full = torch.stack(
            output_pieces, dim=0
        )  # (n_encoder_blocks, seq_len, batch_size, max_embedding_dim)
        return output_full.permute(1, 0, 2, 3)  # (seq_len, n_blocks, bs, dim)
