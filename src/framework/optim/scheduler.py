...
class Eden3(LRScheduler):
    """
    Eden3 scheduler, simpler than Eden because it does not use the notion of epoch,
    only batches.
    ...
    def get_lr(self):
        b = self.batch
        b1 = self.lr_batches
        b2 = self.lr_epochs * self.steps_per_epoch  # convert epoch scale to batch scale

        factor = ((b**2 + b1**2) * (b**2 + b2**2) / (b1**2 * b2**2)) ** -0.25
        warmup_factor = (
            1.0
            if self.batch >= self.warmup_batches
            else self.warmup_start
